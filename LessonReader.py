...
class LessonSpider(scrapy.Spider):
    ...
    def parse(self, response):
        licao_corrente = response.css('licao-corrente.licoes').extract_first()
        json_data_str = re.search(r'\[(.*?)\]', licao_corrente).group(0)
        json_data = json.loads(json_data_str)
        url_licao_corrente = json_data[0].get('link', None)
        yield scrapy.Request(url=url_licao_corrente, callback=self.parse_licao)
...
crawler_process.crawl(LessonSpider)
