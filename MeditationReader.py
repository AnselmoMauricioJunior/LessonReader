...
class MeditationSpider(scrapy.Spider):
    name = 'meditacao'
    start_urls = ['https://mais.cpb.com.br/meditacoes-diarias/']

    def parse(self, response):
        meditacao = response.css('div').extract_first()             
        url_meditacao = re.search(r"post_type=meditacao&amp;p=([0-9]+)", meditacao).group(0)
        url_meditacao = url_meditacao.replace('amp;','')
        url_meditacao='https://mais.cpb.com.br/?'+url_meditacao
        yield scrapy.Request(url=url_meditacao, callback=self.parse_meditacao)
               
        
    def parse_meditacao(self,response):
        
        all_divs = response.css('div').extract_first() 
...
crawler_process.crawl(MeditationSpider)
